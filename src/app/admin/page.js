...
import AdminGroupList from "@/components/admin/group";
...
export default function AdminDashboard() {
  ...
  const { data, isSuccess, isLoading, isFetching, refetch } = useQuery({
    ...
    queryFn: async () => api.get(`/api/v1/bot/get-groups/page?page=${page}&size=${size}`).then(({ data }) => {
  ...
  return (
    <AdminContainer title={t("admin.sidebar.dashboard")}>
      <section id="group" className="content">
        <div id="table" className="group-table">
          <div id="headline">
            <h3>{t("admin.groups.tableTitle")}</h3>
             <p>
          ...
          <ul id="column">
            ...
            <li>
              <span>{t("admin.groups.id")}</span>
            </li>
            <li>
              <span>{t("admin.groups.link")}</span>
            </li>
            <li>
              <span>{t("admin.groups.type")}</span>
            </li>
            <li>
          ...
          {data?.content?.map((group) => (
            <AdminGroupList key={group.id} group={group} refetch={refetch} />
          ))}
